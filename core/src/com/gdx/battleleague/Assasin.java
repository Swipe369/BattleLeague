    ...
    public void makeTurn(final Cell currentCell, final Cell destinationCell, final Cell[][] map) {
        if (destinationCell.isEmpty())
            ...
            timer.scheduleTask(new Timer.Task() {
                @Override
                public void run() {
                    if ((InputHandler.calcX()-1>=0&&InputHandler.calcXpx() <= destinationCell.getX() * Constants.CELL_SIZE + 50) && (InputHandler.calcXpx() % Constants.CELL_SIZE + InputHandler.calcYpx() % Constants.CELL_SIZE <= Constants.CELL_SIZE) && (InputHandler.calcXpx() % Constants.CELL_SIZE <= InputHandler.calcYpx() % Constants.CELL_SIZE))
                        move(currentCell, map[destinationCell.getX() - 1][destinationCell.getY()]);
                    else if ((InputHandler.calcX()+1<=Constants.FIELD_WIDTH-1 &&InputHandler.calcXpx() > destinationCell.getX() * Constants.CELL_SIZE + 50) && (InputHandler.calcXpx() % Constants.CELL_SIZE >= InputHandler.calcYpx() % Constants.CELL_SIZE) && (InputHandler.calcXpx() % Constants.CELL_SIZE + InputHandler.calcYpx() % Constants.CELL_SIZE >= Constants.CELL_SIZE))
                        move(currentCell, map[destinationCell.getX() + 1][destinationCell.getY()]);
                    else if (InputHandler.calcY()+1<=Constants.FIELD_HEIGHT-1&&InputHandler.calcYpx() > destinationCell.getY() * Constants.CELL_SIZE + 50)
                        move(currentCell, map[destinationCell.getX()][destinationCell.getY() + 1]);
                    else if (InputHandler.calcY()-1>=0&&InputHandler.calcYpx() < destinationCell.getY() * Constants.CELL_SIZE + 50)
                        move(currentCell, map[destinationCell.getX()][destinationCell.getY() - 1]);
                }
            }, 0);
